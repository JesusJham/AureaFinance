...
def guardar_data_entry(db: Session, data, usuario_id: int):
    ...
    existe = db.query(DataEntry).filter(
        DataEntry.usuario_id == usuario_id,
        DataEntry.servidor_id == data.servidor_id,
        DataEntry.activo == True,
        ((DataEntry.esquema_raw == data.esquema_raw) &(DataEntry.nombre_tabla_raw == data.nombre_tabla_raw)) |
        ((DataEntry.esquema_silver == data.esquema_silver) &(DataEntry.nombre_tabla_silver == data.nombre_tabla_silver))
    ).first()

    if existe:
        raise ValueError(
            "Ya existe una configuración activa para esa tabla RAW o SILVER"
        )
...
async def cargar_archivo_existente(db, data_entry_id: int, archivo, usuario_id: int):
    ...
    try:
        ...
        if entry.tipo_carga == "FULL":
            ...
            periodos = df[entry.columna_periodo].dropna().unique().tolist()

            if periodos:
                cur.execute(
                    f'''
                    DELETE FROM "{entry.esquema_silver}"."{entry.nombre_tabla_silver}"
                    WHERE "{entry.columna_periodo}" = ANY(%s)
                    ''',
                    (periodos,)
                )
